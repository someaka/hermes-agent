...
def test_matrix_extra_linux_only_in_all():
    ...
    # Verify the Python version guard is present on the gated entry.
    assert any(
        "python_version < '3.13'" in dep for dep in linux_gated
    ), "expected python_version marker on [matrix] in [all]"
